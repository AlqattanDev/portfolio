  ...
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      const validationProgress = Math.min(
        1,
        Math.max(0, (time - validationDelay) / 30)
      );
      ...
      if (validationProgress > 0) {
        ...
        const hashIndex =
          Math.floor(time / 4 + particle.index) % hashChars.length;
        ...
        const pulseIntensity =
          Math.sin(time * 0.2 + particle.index * 0.1) * 0.3 + 0.7;
        particle.offsetY =
          Math.sin(time * 0.1 + validationProgress * Math.PI) * pulseIntensity;
        particle.offsetX =
          Math.cos(time * 0.08 + particle.index * 0.2) *
...
class TradingEffect implements EffectStrategy {
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      particle.color =
        baseColor.replace(/[0-9a-f]{2}/g, (match) => {
  ...
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      if (isNode) {
        ...
        particle.char =
          nodes[Math.floor(time / 10 + particle.index) % nodes.length] ||
        ...
        if (Math.floor(routePhase * 0.5) % 2 === 0) {
          particle.char =
            currencies[
              Math.floor(time / 6 + particle.index) % currencies.length
            ] || particle.originalChar;
          ...
          particle.char =
            flows[Math.floor(time / 8 + particle.index) % flows.length] ||
            particle.originalChar;
...
class RiskAssessmentEffect implements EffectStrategy {
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      particle.char =
        charSet[
          Math.floor(time / 5 + particle.index + totalRisk * 10) %
            charSet.length
        ] || particle.originalChar;
      ...
      particle.offsetX =
        Math.sin(time * volatility * 0.1 + particle.index * 0.3) *
        (totalRisk * 4 + 1);
      particle.offsetY =
        Math.cos(time * volatility * 0.08 + particle.index * 0.2) *
        (totalRisk * 3 + 1);
  ...
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      const isBeingScanned =
        Math.abs(scanProgress - (particle.baseY % 60)) < 10;
      ...
      if (isBeingScanned) {
        ...
        setTimeout(() => {
          if (scanResult < 7) {
            // Passed compliance
            particle.char =
              checkmarks[scanResult % checkmarks.length] ||
              particle.originalChar;
            particle.color = '#00ff88'; // Success green
          } else {
            // Failed compliance
            particle.char =
              warnings[scanResult % warnings.length] || particle.originalChar;
        ...
        particle.char =
          legal[Math.floor(time / 12 + particle.index) % legal.length] ||
          particle.originalChar;
  ...
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      const settlementCycle =
        (time * 0.02 + particle.index * 0.1) % (Math.PI * 6); // 3-day cycle
      ...
      switch (day) {
        case 0: {
          // T+0 - Trade execution
          particle.char =
            matching[Math.floor(time / 8 + particle.index) % matching.length] ||
            particle.originalChar;
        ...
        case 1: {
          ...
          particle.char =
            clearing[
              Math.floor(time / 10 + particle.index) % clearing.length
            ] || particle.originalChar;
        ...
        case 2: {
          ...
          particle.char =
            settlement[
              Math.floor(time / 12 + particle.index) % settlement.length
            ] || particle.originalChar;
    ...
    if (isHovering) {
      ...
      switch (phase) {
        case 0: {
          ...
          particle.char =
            sourceCode[
              Math.floor(time / 4 + particle.index * chaos) % sourceCode.length
            ] || particle.originalChar;
          particle.color = '#fe8019'; // Orange for Rust/source code

          // Code scanning wave
          const scanWave =
            Math.sin(time * 0.3 + particle.index * 0.2) * intensity;
          particle.offsetX = scanWave * 3;
          particle.offsetY =
            Math.cos(time * 0.25 + particle.index * 0.15) * 1.5;
          break;
        }
        ...
        case 1: {
          ...
          particle.char =
            buildSteps[
              Math.floor(time / 6 + particle.index + chaos * 3) %
                buildSteps.length
            ] || particle.originalChar;
          ...
          if ((particle.index + Math.floor(time / 8)) % 15 === 0) {
            ...
            particle.offsetX =
              Math.sin(time * 0.8 + particle.index) * chaos * 2;
            particle.offsetY =
              Math.cos(time * 0.6 + particle.index) * chaos * 1.5;
          }
          break;
        }
        ...
        case 2: {
          ...
          particle.char =
            memoryManagement[
              Math.floor(time / 5 + particle.index * chaos) %
                memoryManagement.length
            ] || particle.originalChar;
          ...
          if (memoryLeak) {
            ...
            particle.offsetX =
              Math.cos(time * 0.2 + particle.index * 0.4) * intensity * 2;
            particle.offsetY =
              Math.sin(time * 0.15 + particle.index * 0.3) * intensity;
          }
          break;
        }
        ...
        case 3: {
          ...
          particle.char =
            debugSymbols[
              Math.floor(time / 7 + particle.index * chaos * 1.5) %
                debugSymbols.length
            ] || particle.originalChar;
          ...
          if (isBreakpoint) {
            ...
            particle.offsetX =
              Math.cos(time * 0.3 + particle.index) * pulse * intensity * 2;
            particle.offsetY =
              Math.sin(time * 0.3 + particle.index) * pulse * intensity;
          } else {
            // Code execution flow
            ...
            const stepPhase =
              (time * 0.8 + particle.index * 0.1) % (Math.PI * 4);
        ...
        case 4: {
          ...
          particle.char =
            optimization[
              Math.floor(time / 3 + particle.index * chaos * 2) %
                optimization.length
            ] || particle.originalChar;
          ...
          const optimizationTarget =
            Math.sin(time * 0.1 + particle.index * 0.05) * intensity * 5;
          ...
          particle.offsetY =
            Math.cos(time * 0.12 + particle.index * 0.08) * intensity * 1.5;
        ...
        case 5: {
          ...
          particle.char =
            deployment[
              Math.floor(time / 5 + particle.index * chaos) % deployment.length
            ] || particle.originalChar;
          ...
          particle.offsetY =
            Math.cos(dancePhase * 1.3) * celebrationIntensity * 0.8;
      ...
      particle.char =
        idleSymbols[
          Math.floor(time / 20 + particle.index) % idleSymbols.length
        ] || particle.originalChar;
  ...
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      if (isSelected) {
        // Inside selection
        particle.char =
          selection[Math.floor(time / 4 + particle.index) % selection.length] ||
          particle.originalChar;
        particle.color = '#fe8019'; // Gruvbox orange for selection

        // Selection highlight pulse
        const pulseIntensity =
          (Math.sin(time * 0.2 + particle.index * 0.1) + 1) * 0.5;
        ...
        particle.char =
          boundaries[
            Math.floor(time / 6 + particle.index) % boundaries.length
          ] || particle.originalChar;
...
class GruvboxSyntaxHighlightEffect implements EffectStrategy {
  ...
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering) {
      ...
      const keywords = {
        chars: ['class', 'def', 'if', 'else', 'for', 'while'],
        color: '#9d0006',
      }; // Red
      const strings = {
        chars: ['"hello"', "'world'", '`template`', 'f"format"'],
        color: '#79740e',
      }; // Green
      const numbers = {
        chars: ['42', '3.14', '0xFF', '1e-6', '0b101'],
        color: '#af3a03',
      }; // Orange
      const comments = {
        chars: ['# comment', '// note', '/* block */'],
        color: '#928374',
      }; // Gray
      const operators = {
        chars: ['=', '+', '-', '*', '/', '&&', '||', '!='],
      ...
      const functions = {
        chars: ['print()', 'map()', 'filter()', 'reduce()'],
        color: '#8f3f71',
      }; // Purple

      // Syntax parsing wave - simulates real-time parsing
      this.parseState.parsingPhase =
        (time * 0.2 + particle.index * 0.1) % (Math.PI * 2);
      ...
      particle.char =
        tokenData.chars[
          Math.floor(time / 15 + particle.index) % tokenData.chars.length
        ] || particle.originalChar;
      ...
      particle.offsetX =
        Math.sin(time * 0.08 + particle.index * 0.25) * highlightIntensity;
      particle.offsetY =
        Math.cos(time * 0.06 + particle.index * 0.2) *
...
class SimpleEffect implements EffectStrategy {
  ...
  apply(particle: CanvasParticle, context: EffectContext): void {
    ...
    if (isHovering && this.config.chars) {
      const charIndex =
        Math.floor(time / 8 + particle.index) % this.config.chars.length;
...
export class EffectSystem {
  ...
  private initializeEffects(): void {
    ...
    this.effects.set(
      4,
      new SimpleEffect({
        // Distributed Ledger (keep existing)
        chars: ['⟨', '⟩', '⟪', '⟫', '⟬', '⟭', '⟮', '⟯'],
        color: '#45b7d1',
        movement: (particle, time) => {
          ...
          particle.offsetY =
            Math.cos(time * 0.04 + particle.index * 0.15) * 1.5;
        },
      })
    );
